...
// Figures every view of a bill agrees on: the rounded total, what came back,
// what is still owed. Shared by the sheet and the share message.
export function invoiceFigures(invoice: Invoice) {
  ...
  return { paid, returned, refunded, exactTotal, total, netPayable, balance };
}

// The printable tax invoice: shop header, lines (with "Added" / "Exchange"
// tags), returns, totals down to the balance due, and payment history. Used
// by the in-app print page and by the public share link.
export default function InvoiceSheet({ invoice, business }: { invoice: Invoice; business: Business }) {
  const isSale = invoice.type === "SALE";
  const tax = Number(invoice.taxAmount);
  const { paid, returned, refunded, exactTotal, total, netPayable, balance } = invoiceFigures(invoice);
  ...
  // Bill rounded to the nearest rupee like a standard GST tax invoice.
